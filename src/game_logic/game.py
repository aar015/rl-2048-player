"""Code courtesy of  georgwiese from https://github.com/georgwiese/2048-rl
     All code in this file is georgwiese's original code except where changes have been explicitly noted with a comment beggining with a *
...
ACTION_RIGHT = 2
ACTION_DOWN = 3
# *Define LENGTH variable to make resizing board easier
LENGTH = 4
...
class Game(object):
    ...
    def __init__(self, state=None, initial_score=0):
        ...
        Args:
          state: Shape (LENGTH, LENGTH) numpy array to initialize the state with. If None
              the state will be initialized with with two random tiles (as done
              in the original game).
          initial_score: Score to initialize the Game with."""
        self._score = initial_score
        if state is None:
            # *Use LENGTH variable instead of hardcoding a 4
            self._state = np.zeros((LENGTH, LENGTH), dtype=np.int)
    ...
    def do_action(self, action):
        ...
        self._score += reward
        # *don't add random tile here, that is done in the play file now
        return reward

    def _do_action_left(self, state):
        """Exectures action 'Left'."""
        reward = 0
        # *Use LENGTH variable instead of hardcodinga 4
        for row in range(LENGTH):
            # Always the rightmost tile in the current row that was already moved
            merge_candidate = -1
            merged = np.zeros((LENGTH,), dtype=np.bool)
