...
class QAgent(Agent):
    ...
    def __init__(self, mask, a=0.0025, g=0.99, e=0.01, name='q'):
    ...
    def learn(self, prevState, action, state, reward): 
        ...
        next_action = numpy.argmax(numpy.sum([self.tuples[num] for num in tupleNums], axis=0))
        qError = self.alpha*(reward+self.gamma*self.getVofSA(state, next_action)-self.getVofSA(prevState, action))
